classdef FieldOfView
    ...
    methods
        ...
        % the folder
        function obj = FieldOfView(ch_names,ch_short_names,folder_path)
            ...
            % detect z_max from file existence
            obj.Z_max = 1;
            while exist([obj.Folder_path '/img_000000000_' obj.Ch_names{1} '_' sprintf('%03i',obj.Z_max-1) '.tif'],'file')
                obj.Z_max = obj.Z_max+1;
            end
            obj.Z_max = obj.Z_max-1;
            % detect t_max from file existence
            obj.T_max = 1;
            while exist([obj.Folder_path '/img_' sprintf('%09i',obj.T_max-1) '_' obj.Ch_names{1} '_000.tif'],'file')
                obj.T_max = obj.T_max+1;
            end
            obj.T_max = obj.T_max-1;
        end
        ...
        function im = loadImage(obj,t,z,ch_short_name,normalize_opt)
            ...
            % handle 'special' z, i.e. min, max, min-max,... encoded by
            % non-positive z
            if (z<1)
                if (z==-1) % z-max
                    im = obj.loadZMaxCroppedImage(t,ch_short_name,[1 1 obj.Width obj.Height],normalize_opt);
                end
                if (z==-2) % z-min
                    im = obj.loadZMinCroppedImage(t,ch_short_name,[1 1 obj.Width obj.Height],normalize_opt);
                end
                if (z==-3) % z-min x z-max
                    im = obj.loadZMinZMaxProductCroppedImage(t,ch_short_name,[1 1 obj.Width obj.Height]);
                end
                if (z==0) % CV
                    im = obj.loadZCvCroppedImage(t,ch_short_name,[1 1 obj.Width obj.Height]);
                end
            % normal z-slice
            else
                for ch=1:obj.N_ch
                    if strcmp(ch_short_name,obj.Ch_short_names{ch})
                        im = imread([obj.Folder_path '/img_' sprintf('%09i',t-1) '_' obj.Ch_names{ch} '_' sprintf('%03i',z-1) '.tif']);
                    end
                end
