...
for aa=5:6
    ...
    for bb=1:indSize(aa)
        %swapping must be done here because this part desn't rely on csv files
        if      aa>10 && aa<13;     cc2 = cmlSize(aa+2)+bb;
        elseif  aa>12 && aa<15;     cc2 = cmlSize(aa-2)+bb;
        else;                       cc2 = cmlSize(aa)+bb;
        end
        cc = cmlSize(aa)+bb;
        ...
        %additional manual changes - righe necessarie ma da rimuovere prima della pubblicazione
            %per qualche motivo, in (gly) la maschera brightfield è più larga in 1 replica che nelle altra
            tComp = strcmp(nameIN(1:5),{'Gly B'});    tSub = [1];
            if any(tComp)
                chSub = tSub(tComp);
                tmatOUT(:,1) = num2cell(cell2mat(tmatOUT(:,1))-chSub*cell2mat(tmatOUT(:,2)));
                tmatOUT(:,3) = num2cell(cell2mat(tmatOUT(:,3))-chSub);
                tmatOUT(:,4) = num2cell(cell2mat(tmatOUT(:,4))-chSub);
                tmatOUT(:,5) = num2cell(cell2mat(tmatOUT(:,5))-chSub*cell2mat(tmatOUT(:,6)));
                tmatOUT(:,7) = num2cell(cell2mat(tmatOUT(:,7))-chSub);
            end 
            %su thrA, rimuovo cellule troppo piccole
            if strcmp(nameIN(1:5),{'Thr A'});   tmatOUT( tmatIN(:,14)<10,15) ={0}; end
        ...
        %correcting mask differences between ideas and matlab
            tmatOUT(:,1)    = cellfun(@(x) x/1.1760 - 3.4185,tmatOUT(:,1),'un',0); %cell area
            tmatOUT(:,2)    = cellfun(@(x) x/0.9429 - 2.9558,tmatOUT(:,2),'un',0); %cell length
            tmatOUT(:,3)    = cellfun(@(x) x/0.9374 - 0.5314,tmatOUT(:,3),'un',0); %cell width_avg
            tmatOUT(:,4)    = cellfun(@(x) x/0.9374 - 0.5314,tmatOUT(:,4),'un',0); %cell width_max
            tmatOUT(:,5)    = cellfun(@(x) x/1.1760 - 3.4185,tmatOUT(:,5),'un',0); %dye area
            tmatOUT(:,6)    = cellfun(@(x) x/0.9429 - 2.9558,tmatOUT(:,6),'un',0); %dye length
            tmatOUT(:,7)    = cellfun(@(x) x/0.9374 - 0.5314,tmatOUT(:,7),'un',0); %dye_width
            %tmatOUT(:,8)   = cellfun(@(x) x/0.9429 - 2.9558,tmatOUT(:,8),'un',0); %dye_delta
            %tmatOUT(:,14)  = cellfun(@(x) x/0.9429 - 2.9558,tmatOUT(:,14),'un',0);%nuclei_delta
...
for aa=1:length(list1)
    ...
    %correcting mask differences between ideas and matlab
        matOUT(:,1)    = cellfun(@(x) x/1.0347 - 6.3489, matOUT(:,1),'un',0); %cell area
        matOUT(:,2)    = cellfun(@(x) x/0.9178 - 3.0672, matOUT(:,2),'un',0); %cell length
        matOUT(:,3)    = cellfun(@(x) x/0.8065 - 1.2689, matOUT(:,3),'un',0); %cell width_avg
        matOUT(:,4)    = cellfun(@(x) x/0.8065 - 1.2689, matOUT(:,4),'un',0); %cell width_max
        %matOUT(:,14)  = cellfun(@(x) x/0.9178 - 3.0672, matOUT(:,14),'un',0);%nuclei_delta
...
for aa=1:3 %1:length(list1)
    ...
    %additional manual changes - righe necessarie ma da rimuovere prima della pubblicazione
        %per qualche motivo, in gly e phe i nuclei autofluorescono in rosso, quindi devo usare una threshold più alta per le mitotiche
        tComp = strcmp(nameIN(3:5),  {'Gly'});              if tComp;	matOUT(matIN(:,7)<=1.85,17) = {0};  matOUT(matIN(:,4)==1 & matIN(:,7)<=1.85,18) = {1};  end
        tComp = strcmp(nameIN(3:end),{'Phe A'});            if tComp;	matOUT(matIN(:,7)<=1.60,17) = {0};  matOUT(matIN(:,4)==1 & matIN(:,7)<=1.60,18) = {1};  end
        tComp = strcmp(nameIN(3:end),{'Phe B'});            if tComp;   matOUT(matIN(:,7)<=1.70,17) = {0};  matOUT(matIN(:,4)==1 & matIN(:,7)<=1.70,18) = {1};  end
        %per qualche motivo, in trp max_width è mezzo pixel più grande del previsto (non si accorda con mean_width e l'andamento con rate e ISX etc)
        tComp = strcmp(nameIN(3:5),{'Trp'});                if tComp;   matOUT(:,4) = num2cell(cell2mat(matOUT(:,4))-0.5);  end

        %su phe, rimuovo cellule troppo piccole
        if strcmp(nameIN(3:5),'Phe');           matOUT(matIN(:,10)<10,15) = {0};  end     
        %restoring consistency between ideas and matlab filter
        tX = matIN(matIN(:,4)>0,10);    tY = tX(tX>9);  
        if (mean(tY)-mean(tX))/mean(tX) >0.01;  matOUT(matIN(:,10)<9,:) = [];   end    
        %per qualche motivo, in (gly,nh4,thr) la maschera brightfield è più larga in 1 replica che nelle altre 2
        tComp = strcmp(nameIN(3:end),{'Gly B','NH4 C','Thr B'});    tSub = [1,1.5,2];
        if any(tComp)
            chSub = tSub(tComp);
            matOUT(:,1) = num2cell(cell2mat(matOUT(:,1))-chSub*cell2mat(matOUT(:,2)));
            matOUT(:,3) = num2cell(cell2mat(matOUT(:,3))-chSub);
            matOUT(:,4) = num2cell(cell2mat(matOUT(:,4))-chSub);
        end
    ...
    %correcting mask differences between ideas and matlab
        ...
        matOUT(:,3)    = cellfun(@(x) x/0.8065 - 1.2689, matOUT(:,3),'un',0); %cell width_avg
        matOUT(:,4)    = cellfun(@(x) x/0.8065 - 1.2689, matOUT(:,4),'un',0); %cell width_max
        matOUT(:,14)   = cellfun(@(x) x/0.9967 - 0.3440, matOUT(:,14),'un',0);%nuclei_delta
